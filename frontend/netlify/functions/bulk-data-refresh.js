/**
 * Bulk Data Refresh - Scheduled Trigger
 *
 * This scheduled function triggers the background worker.
...
exports.handler = async (event, context) => {
  console.log('⏰ Scheduled bulk data refresh - triggering background worker');
  console.log('Time:', new Date().toISOString());

  try {
    // Trigger the background function (fire and forget)
    const response = await fetch(`${SITE_URL}/.netlify/functions/bulk-data-refresh-background`, {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      return {
        statusCode: 200,
        body: JSON.stringify({ success: true, message: 'Background refresh triggered' })
      };
    } else {
